	...
	struct OptimizationSetChooserItem : StatefulWidget {
		...
		struct State : WidgetState<OptimizationSetChooserItem> {
			Child build(const Element &element) override {
				const auto &data = Disc::sets.at(widget->set);
					...
					.children{
						Image{
							...
							.image = ImageProvider::fromFile(
								std::format(
									"assets/Discs/{}/icon.png",
									Disc::sets.at(widget->set).name
								)
							)
						},
						Container{
				...
				Child opts = Column{
					...
					.children = [&]() {
						...
						auto instanceKey = widget->agent.instanceKey;
...
void UI::OptimizationSetChooser::State::setSet(std::unordered_map<Disc::SetKey, bool> &setMap, bool value) {
	setState([&]() {
