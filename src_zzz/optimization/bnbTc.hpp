...
namespace Optimization {
	static inline void bnbTc(
		...
		loadout.disc.sheet.equippedDiscs[1] = &maxStats;
		...
		if (filter.getCombCount() <= 32) {
			auto &equipped = loadout.disc.getTheorycraft();
		...
		bnbTc(copy1, solution, agent, rolls, ctx, node, splitSlotRet);
		bnbTc(copy2, solution, agent, rolls, ctx, node, splitSlotRet);
	}
}// namespace Optimization