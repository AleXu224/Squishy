...
namespace Optimization {
	struct Bnb {
		...
		struct SplitState {
			std::array<std::optional<float>, 6> variances{};
			std::array<Disc::Instance *, 6> worstDiscs{};
			std::optional<uint32_t> splitSlot = std::nullopt;

			SplitState() : variances{}, splitSlot(std::nullopt) {}
		};
		...
		inline void bnb(FilteredDiscs &discs, BnbState &state, SplitState splitState = SplitState{}) {
			...
			for (size_t i = 0; i < 6; i++) {
				if (splitState.splitSlot.has_value() && splitState.splitSlot.value() == i) {
					continue;
				}

				if (splitState.worstDiscs.at(i) != nullptr) {
					loadout.disc.sheet.equippedDiscs.at(i) = &splitState.worstDiscs.at(i)->stats;
				...
				artis.erase(
					std::remove_if(artis.begin(), artis.end(), [&](Disc::Instance *disc) {
						...
						if (val > solutions.minScore) {
							...
							if (val < minVal) {
								minVal = val;
								splitState.worstDiscs.at(i) = disc;
							}
						} else {
				...
				splitState.variances.at(i) = variance;
			}
			...
			for (size_t i = 0; i < 6; i++) {
				...
				if (splitState.variances.at(i).has_value() && splitState.variances.at(i).value() > maxVariance) {
					maxVariance = splitState.variances.at(i).value();
					maxVarianceSlot = i;
				}
			}
			...
			splitState.splitSlot = maxVarianceSlot;
			splitState.variances.at(maxVarianceSlot) = std::nullopt;
			splitState.worstDiscs.at(maxVarianceSlot) = nullptr;
