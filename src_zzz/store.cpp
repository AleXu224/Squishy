...
Agent::Instance &Store::createAgent(Agent::DataKey dataKey, Engine::InstanceKey engine) {
	auto &data = Agent::list.at(dataKey);

	++lastAgentId;
	auto entry = agents.emplace(lastAgentId, Agent::Instance(lastAgentId, dataKey));
	entry.first->second.state.equippedLoadout.swapEngine(engine);
...
// 	std::vector<Serialization::Good::IDisc> discs;
// 	for (const auto &[_, disc]: ::Store::discs) {
// 		discs.emplace_back(Serialization::Good::IDisc::fromInstance(disc));
// 	}

// 	std::vector<Serialization::Good::IEngine> engines;
// 	for (const auto &[_, engine]: ::Store::engines) {
// 		auto weps = Serialization::Good::IEngine::fromInstance(engine);
// 		engines.insert(engines.end(), weps.begin(), weps.end());
// 	}

// 	std::vector<Serialization::Good::IAgent> agents;
// 	for (const auto &[_, agent]: ::Store::agents) {
// 		agents.emplace_back(Serialization::Good::IAgent::fromInstance(agent));
// 	}
...
// 	if (data.agents.has_value()) {
// 		for (const auto &agent: data.agents.value()) {
...
// 	}
// 	if (data.engines.has_value()) {
// 		for (const auto &engine: data.engines.value()) {
