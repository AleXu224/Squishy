...
UI::CharacterEditor::operator Child() const {
	...
	auto levelSelector = NumberBox{
		...
		.value = static_cast<float>(storage->character.loadout.character.sheet.level),
		.min = 1.f,
		.max = static_cast<float>(Misc::ascensions.at(Misc::maxAscensionByRarity.at(storage->character.loadout.character.data.baseStats.rarity)).maxLevel),
		.onChange = [storage, levelUpdateEvent](float newVal) {
			storage->character.loadout.character.sheet.level = std::floor(newVal);
			levelUpdateEvent.notify(storage->character.loadout.character.sheet.level);
		},
		.validator = [](float val) {
	...
	auto ascensionItemFactory = [storage, ascensionUpdateEvent]() {
		...
		auto rarity = storage->character.loadout.character.data.baseStats.rarity;
		for (const auto &ascension: Misc::ascensionsAtLvl(storage->character.loadout.character.sheet.level, rarity)) {
	...
	auto ascensionSelectorText = fmt::format("{}", Misc::ascensions.at(storage->character.loadout.character.sheet.ascension).maxLevel);
			...
			.onInit = [ascensionUpdateEvent, ascensionTextUpdater, storage, levelUpdateEvent, ascensionItemUpdater, ascensionItemFactory](Widget &w) {
				w.customState.add("levelUpdateEvent", levelUpdateEvent.observe([storage, ascensionItemUpdater, ascensionItemFactory, ascensionUpdateEvent, &w](uint8_t level) {
					...
					if (std::ranges::none_of(availableAscensions, [&](const Misc::Ascension &ascension) {
							return ascension.ascension == storage->character.loadout.character.sheet.ascension;
						})) {
				...
				w.customState.add("ascensionUpdateEvent", ascensionUpdateEvent.observe([ascensionTextUpdater, storage](uint8_t newAscension) {
					storage->character.loadout.character.sheet.ascension = newAscension;
					ascensionTextUpdater.notify(fmt::format("{}", Misc::ascensions.at(storage->character.loadout.character.sheet.ascension).maxLevel));
				}));
			},
		},
			...
			.widget{
				.onInit = [constellationUpdateEvent, storage, constellationTextUpdater](Widget &w) {
					observe(w, constellationUpdateEvent, [storage, constellationTextUpdater](uint8_t newConstellation) {
						storage->character.loadout.character.sheet.constellation = newConstellation;
	...
	auto getTalentSelector = [&](std::string_view name, Stats::Value<uint32_t, 1> TalentType::*talentPtr) {
		...
		auto itemBuilder = [storage, talentUpdateEvent]() {
			std::vector<ContextMenu::Item> ret{};
			const auto &ascension = Misc::ascensions.at(storage->character.loadout.character.sheet.ascension);
		...
		return UI::EditorItem{
			...
			.child = DropdownButton{
				.widget{
					.onInit = [talentUpdateEvent, storage, talentTextUpdater, talentPtr, ascensionUpdateEvent, talentItemUpdater, itemBuilder, name](Widget &w) {
						observe(w, talentUpdateEvent, [storage, talentTextUpdater, talentPtr](uint8_t newTalent) {
							std::invoke(talentPtr, storage->character.loadout.character.sheet.talents).constant = newTalent;
				...
				.style = ButtonStyle::Standard(),
				.text = fmt::format("Lvl {}", std::invoke(talentPtr, storage->character.loadout.character.sheet.talents).constant + 1),
				.items = itemBuilder(),
