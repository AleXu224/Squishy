...
namespace Formula {
	...
	concept EvalFuncLike = requires(T t) {
		{ t(std::declval<const Stats::Loadout &>(), std::declval<const Stats::Team &>()) } -> std::same_as<float>;
	};
	...
	struct Custom {
		...
		[[nodiscard]] inline std::string print(const Stats::Loadout &stats, const Stats::Team &team, Step) const {
			return fmt::format("{:2f}{}", func(stats, team) * (isPercentage ? 100.f : 1.f), isPercentage ? "%" : "");
		}

		[[nodiscard]] inline float eval(const Stats::Loadout &stats, const Stats::Team &team) const {
			return func(stats, team);
		}
	};
}// namespace Formula