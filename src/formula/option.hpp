...
namespace Formula {
	...
	struct impl_GetFloatPassive : FormulaBase<float, Type::constant> {
		...
		[[nodiscard]] float eval(const Context &context) const {
			auto &definition = context.team.options->at(name.hash);
			if (auto *opt = matchingOverride(context.overrides, Utils::hashCombine(0, name.hash), definition)) {
				return std::visit(
					Utils::overloaded{
						[](const bool &active) -> float {
							return active ? 1.f : 0.f;
	...
	[[nodiscard]] inline uint32_t activeCharacterIndex(const Context &context) {
		static constexpr Utils::HashedString key{"activeCharacter"};

		uint32_t ret = ::Option::getIndex(*context.team.options, key, context.team.activeCharacterIndex);
		...
		if (ret >= context.team.characters.size()) return context.team.activeCharacterIndex;
		return ret;
	}
}// namespace Formula