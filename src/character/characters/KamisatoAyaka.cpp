...
const Character::Data Character::Datas::kamisatoAyaka{
	...
	.setup = []() -> Data::Setup {
		...
		auto afterSprintingInfusion = IfElse{
			.requirement = afterSprinting,
			.trueVal = Infusion{.element = Misc::Element::cryo},
			.elseVal = NoInfusion{},
		};

		auto a1Cond = IsActive("ayakaA1");
		auto a1Buff = Requires{.requirement = Requirement::passive1 && a1Cond, .ret = Constant{.value = 0.3f}};

		auto a4Cond = IsActive("ayakaA4");
		auto a4Buff = Requires{.requirement = Requirement::passive2 && a4Cond, .ret = Constant{.value = 0.18f}};

		auto c4Cond = IsActive("ayakaC4");
		auto c4Def = Requires{.requirement = Requirement::constellation4 && c4Cond, .ret = Constant{.value = 0.3f}};

		auto c6Cond = IsActive("ayakaC6");
		auto c6Buff = Requires{.requirement = Requirement::constellation6 && c6Cond, .ret = Constant{.value = 2.98f}};
