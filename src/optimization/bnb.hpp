...
namespace Optimization {
	struct Bnb {
		...
		struct SplitState {
			std::array<std::optional<float>, 5> variances{};
			std::array<Artifact::Instance *, 5> worstArtifacts{};
		...
		inline void bnb(FilteredArtifacts &artifacts, SplitState splitState = SplitState{}) {
			...
			std::array<Stats::SheetSimplified<float>, 5> statsForSlot = getMaxStatsForSlots(artifacts);
			...
			for (size_t i = 0; i < 5; i++) {
				...
				if (splitState.worstArtifacts.at(i) != nullptr) {
					loadout.artifact.sheet.equippedArtifacts.at(i) = &splitState.worstArtifacts.at(i)->stats;
				...
				artis.erase(
					std::remove_if(artis.begin(), artis.end(), [&](Artifact::Instance *artifact) {
						...
						if (val > solutions.minScore) {
							...
							if (val < minVal) {
								minVal = val;
								splitState.worstArtifacts.at(i) = artifact;
							}
						}
			...
			splitState.worstArtifacts.at(maxVarianceSlot) = nullptr;
			...
			bnb(copy1, splitState);
			bnb(copy2, splitState);
		}
	};
}// namespace Optimization